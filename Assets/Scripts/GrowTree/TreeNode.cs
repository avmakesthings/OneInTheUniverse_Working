...
class NodeItem
{
    private int _Id;   //--- Node index in its level
    private Vector3 vec;  //--- This vector
    private Vector3 par;  //--- Parent's position
    private double distinneuron; // Distance within the neuron from the root to the node


    //--- Constructor
    public NodeItem(int Id, Vector3 _par, Vector3 _vec, double _distinneuron)
    {
        this._Id = Id;
        this.vec = _vec;
        this.par = _par;
        this.distinneuron = _distinneuron;
    ...
    public double DISTINNEURON
    {
        get { return this.distinneuron; }
        set { this.distinneuron = value; }
    }
